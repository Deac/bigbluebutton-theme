...
function updateMeetingInfo() {
	$.ajax({
    	type: "GET",
		url: 'activity_helper.jsp?getxml=true',
		dataType: "text/xml",
		cache: false,
...
function createMeetingTable(meeting) {	
	...
	var form = '<FORM id="joinus" target="_parent" NAME="form1" METHOD="GET"><input type="hidden" name="meetingID" value="' + meeting.meetingID + '"/>';
	form += '<input type="hidden" name="moderatorPW" value="' + meeting.moderatorPW + '"/>';
	form += '<input type="hidden" NAME=action VALUE="join">';
	form += 'Enter your name:<INPUT TYPE=text NAME=username VALUE="" class="username" style="float:right;"> <br />';
	form += '<input type="submit" id="joinsubmit" value="Join" style="clear:both;"/></FORM>';
	
	var tableContent = '<table name="' + meeting.meetingID + '" class="hor-minimalist-b" cellspacing="0" summary="The current participants in a meeting"><caption>' + meeting.meetingID + '<caption><tr><th scope="col" abbr="Name">Name</th><th scope="col" abbr="Role">Role</th>';
	...
	if (meeting.attendees.attendee) {
		for (var i in meeting.attendees.attendee) {
			...
			tableContent += '<tr id="' + tableRowId + '"><td style="width:128px;">' + attendee.fullName + '</td><td>' + attendee.role + '</td></tr>';
